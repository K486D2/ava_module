...
static inline size_t fifo_spsc_in(fifo_t *fifo, const void *data, size_t data_size) {
  ...
  if (data_size > avail) {
    ...
    case FIFO_POLICY_OVERWRITE: {
      ...
      atomic_store_explicit(&fifo->out, out + (data_size - avail), memory_order_release);
    } break;
    default:
      break;
    }
