...
typedef struct {
        list_head_t link; // 链表节点
        usz         size; // 块大小 (包含头部)
} mp_blk_t;

typedef struct {
        list_head_t free;         // 空闲块链表头
...
mp_alloc(mp_t *mp, usz cap)
{
        ...
        list_head_t *node;
        LIST_FOR_EACH(node, &mp->free)
        {
                mp_blk_t *free_blk = CONTAINER_OF(node, mp_blk_t, link);
                if (free_blk->size < block_size) // 当前块容量不足则检查下一个
                        continue;

                if (free_blk->size >= block_size + MP_BLOCK_HEADER_SIZE + MP_ALIGN) {
                        // 能切分出一个新的空闲块: 先移除原块
                        list_del(node);

                        // 从剩余空间生成 split_blk 并按地址顺序插回空闲链表
                        mp_blk_t *split_blk = (mp_blk_t *)((u8 *)free_blk + block_size);
                        split_blk->size     = free_blk->size - block_size;
                        free_blk->size      = block_size;

                        list_head_t *iter = mp->free.next;
                        while (iter != &mp->free && (u8 *)CONTAINER_OF(iter, mp_blk_t, link) < (u8 *)split_blk)
                                iter = iter->next;

                        __list_add(&split_blk->link, iter->prev, iter);
                } else
                ...
                // 返回指向用户数据区域的指针 (跳过块头)
                return (u8 *)free_blk + MP_BLOCK_HEADER_SIZE;
        }
...
mp_free(mp_t *mp, void *ptr)
{
        ...
        while (pos != &mp->free && (u8 *)CONTAINER_OF(pos, mp_blk_t, link) < (u8 *)block)
                pos = pos->next;

        __list_add(&block->link, pos->prev, pos);
